...
| 1.3 | 2026-09-26 | 新增 §16.3 M2 任务分解；v0.3.0 交付 Web 界面与三大财务报表 |
| 1.4 | 2026-09-26 | 新增 §16.4 M3 任务分解；v0.4.0 交付 TUI（`theme.py` 未单独设立，配色并入终端层） |
| 1.5 | 2026-09-26 | 新增 §16.5 M4 前半任务分解；v0.5.0 交付预算、对账、定期预览与拼音检索 |
| 1.6 | 2026-09-26 | 新增 §16.6 M4 后半任务分解；v0.6.0 交付成本基础、多币种折算、SQL 子集与瀑布图 |
| 1.7 | 2026-09-26 | 新增 §16.7 稳定化任务分解；v0.7.0 语法冻结、发行名确定为 `knot-ledger`、性能验收达标（100,000 笔 6.17 s） |
...
| 序号 | 任务 | 主题 |
|---|---|---|
| 1 | `core/actions.py` 写操作内核（CLI 与 Web 共用） | 基础设施 |
| 2 | `web/service.py` 数据层：mtime 缓存快照、JSON 载荷、写操作、变更监听 | 服务端 |
| 3 | `web/server.py`：`ThreadingHTTPServer`、中文路由、SSE、静态资源与穿越防护 | 服务端 |
| 4 | `web/static/`：仪表盘、记一笔、流水、科目、报表、导入向导（零依赖前端） | 前端 |
| 5 | 三大财务报表：`balance_sheet`、`income_statement`、`cash_flow_statement` | 报表 |
| 6 | 待分类批量归类（`POST /api/归类`，整块改写 + 规则沉淀） | 流水页 |
| 7 | 安全约束：默认 `127.0.0.1`、非本机必须口令、公网警示 | 安全 |
| 8 | 测试：服务层、HTTP 接口、SSE、口令、穿越防护、导入预览与写入 | 质量 |

## 16.4 M3 任务分解（v0.4.0）

| 序号 | 任务 | 主题 |
|---|---|---|
| 1 | `tui/term.py`：VT 启用、终端尺寸、raw/cooked 模式、按键解析（含 Windows msvcrt） | 终端层 |
| 2 | `tui/screen.py`：行级差异重绘、宽字符占位、光标隐藏与恢复 | 终端层 |
| 3 | `tui/input.py`：按键来源抽象（交互 / 脚本回放）、cooked 行输入 | 终端层 |
| 4 | `tui/widgets/list.py`：滚动列表、翻页、搜索、按来源键保持选中 | 控件 |
| 5 | `tui/widgets/tree.py`：科目树构建、折叠展开、子科目小计 | 控件 |
| 6 | `tui/widgets/form.py`：记账表单、字段导航、必填校验 | 控件 |
| 7 | `tui/widgets/spark.py`：盲文迷你图（2×4 点、8 级） | 控件 |
| 8 | `tui/app.py`：固定三栏布局与键位（a/e//f/t/s/?/q），表单复用 `core.actions` | 主程序 |
| 9 | 测试：屏幕缓冲、盲文、控件、脚本化按键驱动记账与编辑、代码量预算 | 质量 |

## 16.5 M4 前半任务分解（v0.5.0）

| 序号 | 任务 | 主题 |
|---|---|---|
| 1 | `core/budget.py`：预算 vs 实际聚合、默认取最近月份、超支判定 | 预算 |
| 2 | `cli/commands/budget.py`（`预算`）：表格 + `--json` + 超支提示 | 预算 |
| 3 | `ChartSpec` 新增 `gauge`：`spec_budget_gauge` 与 SVG 渲染；Web「预算进度」 | 预算 |
| 4 | `core/reconcile.py`：账实比对、待对账清单、`mark_cleared`（写 `P`）、`write_assertion` | 对账 |
| 5 | `cli/commands/reconcile.py`（`对账`）：`--余额/--标记/--断言/--json`，差额非 0 退出码 1 | 对账 |
| 6 | `cli/commands/recur.py`（`定期`）：模板列表（含下次发生）与按月展开预览 | 定期 |
| 7 | `core/pinyin.py`：内嵌常用汉字拼音表、全拼/首字母匹配；接入 `AliasTable.resolve` | 拼音 |
| 8 | `别名 列表 --全部`、`别名 测试`；`resolve_account` 贯通三端 | 别名 |
| 9 | 测试：预算/对账/拼音/命令共 19 项 | 质量 |

## 16.6 M4 后半任务分解（v0.6.0）

| 序号 | 任务 | 主题 |
|---|---|---|
| 1 | `core/inventory.py`：FIFO / 移动平均、批次消耗、已实现与未实现盈亏 | 成本基础 |
| 2 | `latest_price` / `convert`：报价查询与折算（直接 / 反向 / 经记账币种） | 多币种 |
| 3 | `net_worth_in`：净资产折算，缺报价单列不猜测 | 多币种 |
| 4 | `cli/commands/holdings.py`（`持仓`）：表格 + `--方法` + `--折算` + `--json` | 命令 |
| 5 | `core/sql.py`：SELECT / WHERE / GROUP BY / ORDER BY / LIMIT，含 `包含` 类运算符与聚合 | 查询 |
| 6 | `查 --sql`：表格与 `--json` 输出 | 查询 |
| 7 | `ChartSpec` 新增 `waterfall`（期初 → 经营 → 投资 → 筹资 → 期末）与 Web 仪表盘面板 | 图表 |
| 8 | Web：`/api/持仓`、`/api/图表/现金流` | Web |
| 9 | 测试：成本基础、折算、SQL、命令共 14 项 | 质量 |

## 16.7 稳定化任务分解（v0.7.0）

| 序号 | 任务 | 主题 |
|---|---|---|
| 1 | 语法冻结：`schema/GRAMMAR.md` 冻结声明，手册/大全/速查同步标注 | 冻结 |
| 2 | 性能：`scripts/性能基准.py` 生成与基准、`tests/test_perf.py` 门槛断言 | 性能 |
| 3 | 分发：`Dockerfile`、`scripts/构建可执行.py`（Nuitka）、CI 工作流（测试矩阵 + zipapp 冒烟 + 三平台可执行） | 分发 |
| 4 | 发布前检查：PyPI 包名可用性核查、发行名调整为 `knot-ledger`、商标待办登记 | 发布 |
